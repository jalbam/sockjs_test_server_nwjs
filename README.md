...
The default listening port is set to 9999 and the directory is "echo", but they can be modified easily. By default, if using the same machine, the following address can be used to connect to it: _http://localhost:9999/echo_ (replace "_localhost_" with the IP if needed).
